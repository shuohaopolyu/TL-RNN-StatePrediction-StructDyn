...
class PSDExcitationGenerator:
    ...
    def __init__(self, psd, tmax, fmax, normalize=False, normalize_factor=1.0):
        """
    ...
    def generate(self):
        ...
        """
        if self.normalize:
            force_series = np.fft.irfft(
                self._calculate_fourier_components(),
                n=self.num_samples,
                axis=0,
                norm="forward",
            )
...
class PeriodicExcitationGenerator:
    ...
    def generate(self):
        ...
        Generate the time and random excitation signal in numpy arraies of dimension (n_t, )
        """
        return self.t, self.amplitude * np.sin(
            2 * np.pi * self.t / self.period + self.phase
        )
